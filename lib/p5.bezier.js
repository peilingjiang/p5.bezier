...
p5bezierAccuracyListAll = [0.2, 0.1, 0.05, 0.04, 0.02, 0.01, 0.008, 0.002, 0.001, 0.0005, 0.0001];

p5.prototype.newBezier = function (pointList, closeType, accuracy = null) {
  closeType = closeType || "OPEN";
  if (accuracy == null)
    accuracy = 7; // DEFAULT FIDELITY
    ...
    if (bezierDimension == 2) {
      ...
      for (t = 0; t <= 1; t += tIncrement) {
        ...
        for (i = 0; i <= n; i++) {
          ...
          x += (_helper_factorial(n) / (_helper_factorial(i) * _helper_factorial(n - i))) * pow(1 - t, n - i) * pow(t, i) * pointList[i][0];
          y += (_helper_factorial(n) / (_helper_factorial(i) * _helper_factorial(n - i))) * pow(1 - t, n - i) * pow(t, i) * pointList[i][1];
        }
        vertex(x, y);
      }
      let tempL = pointList.slice(-1)[0];
      vertex(tempL[0], tempL[1]);
      ...
      for (t = 0; t <= 1; t += tIncrement) {
        ...
        for (i = 0; i <= n; i++) {
          for (d = 0; d < 3; d++) {
            xyz[d] += (_helper_factorial(n) / (_helper_factorial(i) * _helper_factorial(n - i))) * pow(1 - t, n - i) * pow(t, i) * pointList[i][d];
          }
        }
        vertex(xyz[0], xyz[1], xyz[2]);
      }
      let tempL = pointList.slice(-1)[0];
      vertex(tempL[0], tempL[1], tempL[2]);
    }
...
function _helper_factorial(a) {
  ...
  if (a == 0 || a == 1) {
    ...
    return a * _helper_factorial(a - 1);
  }
}
...
class BezierCurve {
    ...
    */
    ...
    if (this.dimension == 2) {
      ...
      for (t = 0; t <= 1; t += this.increment) {
        ...
        for (i = 0; i <= this.n; i++) {
          // i point in pointList
          x += (_helper_factorial(this.n) / (_helper_factorial(i) * _helper_factorial(this.n - i))) * pow(1 - t, this.n - i) * pow(t, i) * this.controlPoints[i][0];
          y += (_helper_factorial(this.n) / (_helper_factorial(i) * _helper_factorial(this.n - i))) * pow(1 - t, this.n - i) * pow(t, i) * this.controlPoints[i][1];
        }
      ...
      for (t = 0; t <= 1; t += this.increment) {
        ...
        for (i = 0; i <= this.n; i++) {
          for (d = 0; d < 3; d++) {
            xyz[d] += (_helper_factorial(this.n) / (_helper_factorial(i) * _helper_factorial(this.n - i))) * pow(1 - t, this.n - i) * pow(t, i) * this.controlPoints[i][d];
          }
        }
    ...
    // Ending fix
    let tempL = this.controlPoints.slice(-1)[0];
    this._addVertex(tempL);
  ...
  draw(dash = 0) {
    if (dash == 0) {
      ...
      let onePart = solidPart + abs(dash[1]),
        nowLen = 0,
        modOnePart = 0;
  ...
  move(x, y, z = null, toDraw = true, dash = 0) {
    ...
    */
    if (z == null && this.dimension == 3) {
      ...
      let newCurveObj = new BezierCurve(this.controlPoints, this.closeType, this.increment, this.dimension, newCurveV);
      // Move
