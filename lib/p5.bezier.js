...
p5.prototype.newBezier = function (pointList, closeType, accuracy) {
  closeType = closeType || "OPEN";
  accuracy = accuracy || 7; // DEFAULT FIDELITY
  ...
  if (!Array.isArray(pointList)) {
    ...
    var bezierDimension = pointList[0].length;
...
p5.prototype.newBezierObj = function (pointList, closeType='OPEN', accuracy=7) {
  ...
  if (!Array.isArray(pointList)) {
    ...
    // All checks done
    var bObj = new BezierCurve(pointList, closeType, tIncrement, bezierDimension);
    return bObj;
  }
}
...
class BezierCurve {
  ...
  draw(dash=0) {
    if (dash == 0) {
      ...
      // Ending fix
      var tempL = this.controlPoints.slice(-1)[0];
      ...
      // Draw a dash curve
      var solidPart = abs(dash[0]); // Length of one solid part
      var onePart = solidPart + abs(dash[1]), nowLen = 0, modOnePart = 0;
